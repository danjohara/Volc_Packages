...
% MorVolc Metrics
mMets.Main_Flank_Average_Slope = NaN;
...
if isstruct(mets)
    ...
    for i = 1:length(DB_IDs)
        ...
        dMets.P_Basin_Width_Distances = mets.DrainageParams.Basin_Statistics(ii,12);
...
if isstruct(morRes)
    ...
    mMets.Total_Skewness = sqrt(morRes.ShapeParams.Skewness(1)^2 + morRes.ShapeParams.Skewness(2));
...
dMets.Norm_P_STD_Basin_Relief = std(dMets.Norm_P_Basin_Reliefs);
dMets.Norm_P_STD_Basin_Length = std(dMets.Norm_P_Basin_Lengths);
dMets.Norm_P_STD_Basin_Width = std(dMets.Norm_P_Basin_Widths);
