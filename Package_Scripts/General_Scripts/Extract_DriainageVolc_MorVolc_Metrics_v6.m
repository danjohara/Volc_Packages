function [dMets,mMets] = Extract_DriainageVolc_MorVolc_Metrics_v6(drainageVolcFile,morVolcFile)
%%
...
if isstruct(mets)
    ...
    %% Basin topology values and distributions for top basins
    [DB,x,y] = GRIDobj2mat(mets.DrainageParams.DB);
    DB_IDs = mets.DrainageParams.TopN_basinIDs;
...
%         ii = find(round(BCCpCLA(:,2),3)==contVals(i));
    ...
    dMets.Basal_Width = sqrt(polyarea(mets.GeneralParams.boundaryXY(:,1),mets.GeneralParams.boundaryXY(:,2))./pi);

    [Z,~,~] = GRIDobj2mat(mets.GeneralParams.DEM);
    dMets.Max_Relief = range(Z(:));
...
if isstruct(morRes)
    ...
    %DEM_S = gradient8(morRes.GeneralParams.DEM);
    %[S,~,~] = GRIDobj2mat(DEM_S);
    S = morRes.GeneralParams.S;
    ...
    % summit cutoff
    mMets.Irregularity_Indices_BFE(mMets.Irregularity_Indices_BFE(:,1)>morRes.GeneralParams.Summit_Contour,2) = NaN;
    mMets.Ellipticity_Indices_BFE(mMets.Ellipticity_Indices_BFE(:,1)>morRes.GeneralParams.Summit_Contour,2) = NaN;
    mMets.Axis_Ellipticities(mMets.Axis_Ellipticities(:,1)>morRes.GeneralParams.Summit_Contour,2) = NaN;
    ...
    % Main Flank
    SS = S;
    SS(Z>morRes.GeneralParams.Summit_Contour) = NaN;
