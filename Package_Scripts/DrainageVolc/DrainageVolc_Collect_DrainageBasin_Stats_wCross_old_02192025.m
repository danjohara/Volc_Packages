function [tot_db_stats,tot_db_grids,cross_db_stats] = DrainageVolc_Collect_DrainageBasin_Stats_wCross(Z,Slp,A,DB,FD,D,usePar,smoothBasinPointWavelength,basinStatThreshold,verbose)
...
%       Euclidean distance between the headwater and channel outlet, 
%       along-basin distance from the headwater to the largest width, and 
%       total slope variance (std / mean).
%   tot_db_grids: Structure of grids for overall basin
%       lengths, heights, widths, orientations (phi), flow lengths, 
%       hypsometry integrals, mean slopes, sinuosities, drainage area, and
%       slope variance.
...
%       (perpendicular to broad basin orientation), and incision ratio 
%       (cross-basin relief / cross-basin width).
...
tot_db_stats = zeros(length(dbi),13);
tot_db_grids = [];
cross_db_stats = [];
...
for i = 1:length(dbi)
    ...
    if ~isnan(basinStatThreshold) && totDArea > basinStatThreshold
        ...
        tmpZ = Z;
        tmpZ(bL==0) = NaN;
        [bas_hyps_areas,bas_hyps_vals] = HypsometryValue(tmpZ,.01,1);
        ...
        % Get slope variance
        stdSlopes = nanstd(tmpS(:));
        varSlopes = stdSlopes./totSlopes;
        ...
        if verbose > 1
            disp(sprintf('         Basin %d below threshold',dbi(i)))
        end
        totBLength = NaN;
        totBWidth = NaN;
        totBOrient = NaN;
    ...
    %% Fill stats matrix
    tot_db_stats(i,:) = [dbi(i),totDArea,totFLength,totBLength,totBWidth,totBHeight,totBOrient,totHyps,totSlopes,totSinuosity,totEucDist,BWidthDistance,varSlopes];
    ...
    tmpSlopeVar.Z(DBg==dbi(i)) = varSlopes;
...
tot_db_grids.BasinSlopeVariance = tmpSlopeVar;
end
