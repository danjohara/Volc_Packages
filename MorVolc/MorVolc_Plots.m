...
try
    ...
    if GP.inputs.interpSurfaces.Kriging
        subplot(2,2,3)
        hold on
        plot3(edXYZ(:,1),edXYZ(:,2),edXYZ(:,3),'.k')
        plot3(GP.BoundaryXYZ(:,1),GP.BoundaryXYZ(:,2),GP.BoundaryXYZ(:,3),'ko','markerfacecolor','b')
        plot3(GP.Basal_Surface_XY(1:scarseInd:end,1),GP.Basal_Surface_XY(1:scarseInd:end,2),GP.Basal_Surface_Z.Kriging(1:scarseInd:end),'.r')
...
try
    ...
    if GP.inputs.interpSurfaces.Kriging
        tx = sortrows([xProfBPoints,GP.Basal_Surface_Z.Kriging(xProfBPointsLog)],1);
        pp = plot(tx(:,1),tx(:,2),'--r','linewidth',2);
        allP = [allP,pp];
    ...
    if GP.inputs.interpSurfaces.Kriging
        tx = sortrows([yProfBPoints,GP.Basal_Surface_Z.Kriging(yProfBPointsLog)],1);
        pp = plot(tx(:,1),tx(:,2),'--r','linewidth',2);
        allP = [allP,pp];
        allPt = [allPt,{'Kriging Interp.'}];
    end
...
try
    ...
    figure('Name','Peak Parameters','units','normalized','outerposition',[0 0 1 1])
    hold on
    imageschs(GP.DEM,[],'colormap',demcmap(GP.DEM.Z))
    plot(GP.BoundaryXYZ(:,1),GP.BoundaryXYZ(:,2),'-k','linewidth',2);
    plot(GP.SummitXYZ(:,1),GP.SummitXYZ(:,2),'-r','linewidth',2);
    plot(GP.Lower_Flank_XY(:,1),GP.Lower_Flank_XY(:,2),'-b','linewidth',2);
    if ~isempty(GP.MaskXY)
        for i = 1:length(GP.MaskXY)
            fill(GP.MaskXY{i}(:,1),GP.MaskXY{i}(:,2),'w');
        end
    end
    if ~isempty(GP.CraterXYZ)
        for i = 1:length(GP.CraterXYZ)
            fill(GP.CraterXYZ{i}(:,1),GP.CraterXYZ{i}(:,2),'r','facealpha',.5);
        end
    end
    ...
    allPT = {};
    % Summit Peaks
    if ~isempty(PP.Summit_Local_Peak_XYZ)
        pp = plot(PP.Summit_Local_Peak_XYZ(:,1),PP.Summit_Local_Peak_XYZ(:,2),'.r','markersize',ms1);
        allP = [allP;pp];
        allPT = [allPT;{sprintf('Summit Local Peak (%d)',PP.Summit_Local_Peak_Count)}];
    end
    if ~isempty(PP.Summit_Contour_Peak_XYZ)
        pp = plot(PP.Summit_Contour_Peak_XYZ(:,1),PP.Summit_Contour_Peak_XYZ(:,2),'ok','markerfacecolor','r','markersize',ms2);
        allP = [allP;pp];
        allPT = [allPT;{sprintf('Summit Contour Peak (%d)',PP.Summit_Contour_Peak_Count)}];
    end

    % Main Flank Peaks
    if ~isempty(PP.Main_Flank_Local_Peak_XYZ)
        pp = plot(PP.Main_Flank_Local_Peak_XYZ(:,1),PP.Main_Flank_Local_Peak_XYZ(:,2),'.b','markersize',ms1);
        allP = [allP;pp];
        allPT = [allPT;{sprintf('Main Flank Local Peak (%d)',PP.Main_Flank_Local_Peak_Count)}];
    end
    if ~isempty(PP.Main_Flank_Contour_Peak_XYZ)
        pp = plot(PP.Main_Flank_Contour_Peak_XYZ(:,1),PP.Main_Flank_Contour_Peak_XYZ(:,2),'ok','markerfacecolor','b','markersize',ms2);
        allP = [allP;pp];
        allPT = [allPT;{sprintf('Main Flank Contour Peak (%d)',PP.Main_Flank_Contour_Peak_Count)}];
    end

    % Lower Flank Peaks
    if ~isempty(PP.Lower_Flank_Local_Peak_XYZ)
        pp = plot(PP.Lower_Flank_Local_Peak_XYZ(:,1),PP.Lower_Flank_Local_Peak_XYZ(:,2),'.k','markersize',ms1);
        allP = [allP;pp];
        allPT = [allPT;{sprintf('Lower Flank Local Peak (%d)',PP.Lower_Flank_Local_Peak_Count)}];
    end
    if ~isempty(PP.Lower_Flank_Contour_Peak_XYZ)
        pp = plot(PP.Lower_Flank_Contour_Peak_XYZ(:,1),PP.Lower_Flank_Contour_Peak_XYZ(:,2),'ok','markerfacecolor','k','markersize',ms2);
        allP = [allP;pp];
        allPT = [allPT;{sprintf('Lower Flank Contour Peak (%d)',PP.Lower_Flank_Contour_Peak_Count)}];
    end
    ...
    setAxes(cb,[]);
    title({'Peak Count';...
        sprintf('(Total Local Peaks: %d)',PP.Summit_Local_Peak_Count+PP.Main_Flank_Local_Peak_Count+PP.Lower_Flank_Local_Peak_Count);...
        sprintf('(Total Contour Peaks: %d)',PP.Summit_Contour_Peak_Count+PP.Main_Flank_Contour_Peak_Count+PP.Lower_Flank_Contour_Peak_Count)})
    legend(allP,allPT,'Location','southwestoutside');
    savePlot(GP,gcf,'Peak_Parameters')
catch 
...
try
    ...
    for i = 1:length(GP.CraterXYZ)
        ...
        if GP.inputs.interpSurfaces.Kriging
            ...
            plot3(tmpPoints(:,1),tmpPoints(:,2),tmpPoints(:,3),'.k')
            plot3(cxyz(:,1),cxyz(:,2),cxyz(:,3),'ko','markerfacecolor','b')
            plot3(res.CraterParams.Crater_Surface_XY{i}(:,1),res.CraterParams.Crater_Surface_XY{i}(:,2),res.CraterParams.Crater_Surface_Z{i}.Kriging,'.r')
