...
if interpSurfaces.Kringing
    ...
    volcHeights.Kringing = peakHeight-basalSurfZ(peakI);
    
    tmp = zzp-basalSurfZ;
    [ii] = find(tmp==max(tmp(:)),1);
    anyHeights.Kringing.X = xxp(ii);
    anyHeights.Kringing.Y = yyp(ii);
    anyHeights.Kringing.Height = tmp(ii);
    
    volcVols.Kringing = sum(abs(basalSurfZ-zzp))*dx^2;
else
    surfZs.Kringing = [];
    volcHeights.Kringing = [];
    volcVols.Kringing = [];
    anyHeights.Kringing = [];
end
